...
#include "memory.h"
#define APP0_START 0x40000000
#define APP1_START 0x40000000
#define APP2_START 0x40000000
...
void init_tasks(){
  ...
  asm volatile ("mov %0, %%cr3" :: "a"(task_cr3s[1]));
  ...
  sp = sp_hold;
  asm volatile ("mov %0, %%cr3" :: "a"(task_cr3s[2]));
  sp = tasks[2].sp;
  sp_hold = sp;
  hoge = APP2_START;
  asm volatile(
  ...
  hoge = APP0_START;
  asm volatile ("mov %0, %%cr3" :: "a"(task_cr3s[0]));
...
void schedule(unsigned long long sp) {
  ...
  unsigned long long cr3 = task_cr3s[current_task];
